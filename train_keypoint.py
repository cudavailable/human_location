...
from utils.DataBuilderKPT import DatasetBuilder

from models.MODEL import MODEL
from models.PnP import CameraCalibrator

import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--log_root_dir', type=str, default='logs_train_kpt', help='root dir of log files')
...
def train(model, alpha, criterion, optimizer, train_loader, device):
    model.train()
    loss_summary = {'total_loss': 0.0, 'loss_X': 0.0, 'loss_x1': 0.0, 'loss_x2': 0.0, 'count': 0}
    for data in train_loader:
        ...
        loss_summary['loss_x1'] += loss_x1.item()
        loss_summary['loss_x2'] += loss_x2.item()
        loss_summary['count'] += 1

    for key in loss_summary:
        if key != 'count':
            loss_summary[key] /= loss_summary['count']
...
def display_acc(logger, train_metrics, test_metrics, train_metrics_history, test_metrics_history, epoch, threshes):
    ...
    for thresh in threshes:
        logger.write(f"Accuracy @ {thresh}m: Train={train_metrics['accuracy'][thresh]*100:.4f}%, Test={test_metrics['accuracy'][thresh]*100:.4f}%\n")
...
def save_loss_data(loss_data, filename):
...
def main(args):
    ...
    for epoch in range(num_epochs):
        train_loss_metrics = train(model, alpha, criterion, optimizer, walking_trainloader, device)
        train_loss_data.append(train_loss_metrics)
        logger.write(f"\nEpoch {epoch}/{num_epochs}, Train Loss: {train_loss_metrics['total_loss']:.4f}")

        train_metrics, _ = evaluate(model, alpha, criterion, walking_trainloader, threshes, device, args)
        test_metrics, test_loss_metrics = evaluate(model, alpha, criterion, walking_testloader, threshes, device, args)
        test_loss_data.append(test_loss_metrics)
    ...
    save_loss_data(train_loss_data, os.path.join(log_dir, 'training_loss_history.csv'))
    save_loss_data(test_loss_data, os.path.join(log_dir, 'testing_loss_history.csv'))
    ...
    for key, value in best_epoch_info['metrics'].items():
        if key != 'accuracy':
            ...
            for thresh in threshes:
                logger.write(f"Best Accuracy @ {thresh}m: {value[thresh]*100:.4f}%\n")

    save_plot_metrics(train_metrics_history, test_metrics_history, vis_dir)
    
    model.load_state_dict(torch.load(best_weight_path))
    model.eval()
    walking_test_metrics, _ = evaluate(model, alpha, criterion, walking_testloader, threshes, device, args)
    cross_train_metrics, _ = evaluate(model, alpha, criterion, cross_trainloader, threshes, device, args)
    cross_test_metrics, _ = evaluate(model, alpha, criterion, cross_testloader, threshes, device, args)
    square_train_metrics, _ = evaluate(model, alpha, criterion, square_trainloader, threshes, device, args)
    square_test_metrics, _ = evaluate(model, alpha, criterion, square_testloader, threshes, device, args)
    
    zhang_dir = os.path.join(log_dir, 'calibration')
    os.makedirs(zhang_dir)
    zhang_calibrator = CameraCalibrator(cam1_cali_path = './init_calibration/cam1_init_calibration.npz', cam2_cali_path = './init_calibration/cam2_init_calibration.npz', num_points_per_sample=num_kpt-args.n_mask)
    zhang_calibrator.calibrate(
        imgpoints1 = walking_trainset.zhang_img0, 
        imgpoints2 = walking_trainset.zhang_img1, 
        objpoints = walking_trainset.zhang_objs, 
        image_size = (args.width, args.height), 
        save_dir = zhang_dir
    )
    
    zhang_walking_test_errors = zhang_calibrator.evaluate(walking_testset.zhang_img0, walking_testset.zhang_img1, walking_testset.zhang_objs)
    zhang_cross_cali_errors = zhang_calibrator.evaluate(cross_trainset.zhang_img0, cross_trainset.zhang_img1, cross_trainset.zhang_objs)
    zhang_cross_test_errors = zhang_calibrator.evaluate(cross_testset.zhang_img0, cross_testset.zhang_img1, cross_testset.zhang_objs)
    zhang_square_cali_errors = zhang_calibrator.evaluate(square_trainset.zhang_img0, square_trainset.zhang_img1, square_trainset.zhang_objs)
    zhang_square_test_errors = zhang_calibrator.evaluate(square_testset.zhang_img0, square_testset.zhang_img1, square_testset.zhang_objs)
    ...
    all_metrics = [
        ...
        ('zhang_walking_test', zhang_walking_test_errors),
        ('zhang_cross_cali', zhang_cross_cali_errors),
        ('zhang_cross_test', zhang_cross_test_errors),
        ('zhang_square_cali', zhang_square_cali_errors),
        ('zhang_square_test', zhang_square_test_errors)
    ]
